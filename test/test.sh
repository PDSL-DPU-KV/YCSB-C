## workload parameters
workload="drop"
workload_array=("drop" "load" "workloadc")
threads=1

## db parameters
db=rocksdb
dbpath="/mnt/ssd/test"
dboptions="./config/template-rocksdb.json"
dbstatistics=1
dbwaitforbalance=1
perflevel=3
...
function copy_out_file() {
    ...
    \cp -f $bench_dir/out.out $res_dir/
    \cp -f $bench_dir/compaction.csv $res_dir/
    \cp -f $bench_dir/flush.csv $res_dir/
}
